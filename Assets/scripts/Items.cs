...
public class Item{
    ...
    public float power; //how many components went into it
    public float stability; //how likely it is to mutate
...
public class ArrayDeck : Deck{
    ...
    public void Render(GameObject panel){
        for (int i=0;i<count;i+=2){
            ...
            o.transform.SetParent(o);
            for (int j=0;j<2;j++){
                if(data[i+j]!=null){
                    data[i+j].RenderCard(o.GetChild(j));
                }
            }
            o.transform.position=new Vector3(-100+ 50*(i),0,0);
        }
    }
}
...
public class HashDeck : Deck{
    ...
    public void Render(GameObject panel){
        for (int i=0;i<count;i+=2){
            ...
            o.transform.position=new Vector3(-100+ 50*(i),0,0); //FIXME the offsets are wrong
        }
    }
