...
// the ui namespace will no longer be used, i am going to use more diagetic
// information schema
namespace Ui {
...
public class Frame {
    ...
    public Frame(Vector2 size, Vector2 position) {
        GameObject gameObject =
            MeshGens.MinObjGen(Shapes.square, MatColour.background);
    ...
    public GameObject Render(GameObject o, Vector2 pos, Vector2 size) {
        ...
        pos.y += size.y / 2;
        float form =
            canvas.transform.lossyScale.y / canvas.transform.lossyScale.x;
        size.y *= form / 0.7f;
        s.x *= size.y * 2;
        s.y *= size.y * 2;
    ...
    public TextMeshPro Render(object o, Vector2 pos, Vector2 size,
                              float ts = 0.6f) {
    ...
    public void Render(List<object> objects, Vector2 pos, Vector2 size) {
        ...
        foreach (object o in objects) {
            TMP_Dropdown.OptionData data =
                new TMP_Dropdown.OptionData(o.ToString());
        ...
        ColorBlock cb =
            new ColorBlock { colorMultiplier = 1,   disabledColor = bg,
                             normalColor = bg,      highlightedColor = bg,
                             pressedColor = bg_alt, selectedColor = bg_alt };
        dropdown.colors = cb;
        Transform label = dropdown.transform.GetChild(0);
        TextMeshProUGUI tmp = label.GetComponent<TextMeshProUGUI>();
        tmp.color = fg;
        sr.GetComponent<Image>().color = bg;
        Toggle item = sr.transform.GetChild(0)
                          .GetChild(0)
                          .GetChild(0)
                          .GetComponent<Toggle>();
        item.colors = cb;
    ...
    public void Render(List<Frameable> frames, string title, Vector2 pos,
                       Vector2 size) {}
