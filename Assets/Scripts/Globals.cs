...
public static class Globals {
    ...
    public static void DealDmgAlongLine(Vector3[] positions, int head,
                                        int layerMask, Run onHit, int peirce,
                                        int length) {
        int hits = 0;
        ...
        for (int i = 0; i < (length - di); i += di) {
            RaycastHit2D[] cols = Physics2D.LinecastAll(
                positions[(i + head) % length],
                positions[(i + head + di) % length], layerMask);
