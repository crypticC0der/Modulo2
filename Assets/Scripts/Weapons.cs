...
namespace Modulo{
	...
	public abstract class Weapon{
		...
		public float timeStep(){
			...
			if((perent.attackProperties() & SpecialProperties.spdUp)!=0){
				if(chargingPercent>0 && s==MeleeState.Attacking || s==MeleeState.MidAttack){
			...
			if(s==MeleeState.Waiting || s==MeleeState.Ready){
				if (t <= 0)
				{
					...
					if (c!=null)
					{
						...
						t= perent.attackRate(rapidFireShotCount,chargingPercent)+ perent.perent.attackSpeed;
	...
	public class Dagger : Weapon{
		...
		public override Collider2D Target(bool smart){
			Collider2D[] cols = Physics2D.OverlapCircleAll(self.transform.position,perent.attackRange(),perent.perent.layerMask(true));
			float best=-1;
			...
			foreach(Collider2D c in cols){
				...
				if(dot>best && (c.transform.position-self.transform.position).magnitude>(perent.shotSpeed()*timeStep()*.1/Time.deltaTime)){
					bcol = c;
					best=dot;
				}
			}
		...
		public override void Update(){
			...
			if(s==MeleeState.Attacking){
				...
				self.transform.eulerAngles=new Vector3(0,0,World.VecToAngle(distance));
				if(distance.magnitude>perent.shotSpeed()*timeStep()){
					self.transform.position+=distance.normalized*perent.shotSpeed()*timeStep();
				}else{
				...
				self.transform.eulerAngles=new Vector3(0,0,180+World.VecToAngle(distance));
				if(distance.magnitude>perent.shotSpeed()*timeStep()){
					self.transform.position+=(Vector3)distance.normalized*perent.shotSpeed()*timeStep();
				}else{
			...
			if(s==MeleeState.Waiting || s==MeleeState.Ready){
				if(!((Vector2)(self.transform.position)!=start && ((perent.attackProperties()&SpecialProperties.homing)!=0))){
				...
				if (t <= 0)
				{
					...
					if(!perent.hasProperties(SpecialProperties.random)){
						c = Target(!((Vector2)(self.transform.position)!=start && ((perent.attackProperties()&SpecialProperties.homing)!=0)));
					}
					if (c!=null || perent.hasProperties(SpecialProperties.random))
					{
						if(perent.hasProperties(SpecialProperties.random)){
							target = Attack.RandomInCircle(perent.attackRange())+perent.perent.transform.position;
						}else{
			...
			if(s==MeleeState.Ready && ((Vector2)(self.transform.position)!=start && ((perent.attackProperties()&SpecialProperties.homing)!=0))){
	...
	public class Tentacle : Weapon{
		...
		void CheckSegments(){
			...
			if(Mathf.Abs(segLen-aimedSegLength)>Mathf.Abs(newLen-aimedSegLength)){
		...
		Collider2D GetTarget(Vector2 center,float radius){
			Collider2D[] cols = Physics2D.OverlapCircleAll(center,radius,perent.perent.layerMask(true));
		...
		public void CollisionUpdate(){
			if(perent.hasProperties(SpecialProperties.random)){
				int total = 1;
				if(perent.hasProperties(SpecialProperties.returning)){total+=1;};
				if(perent.hasProperties(SpecialProperties.homing)){total+=1;};
				randAims = new Vector3[total];
				randAims[total-1]=Attack.RandomInCircle(perent.attackRange())+self.transform.position;
				for(int i=0;i<total-1;i++){
					randAims[i]=Attack.RandomInCircle(perent.attackRange()/3)+r.GetPosition((1+i)*segments/aim.Length);
				}
			}else{
				int add=0;
				if(perent.hasProperties(SpecialProperties.returning)){add=1;};

				int endWay=1;
				if((perent.attackProperties() & SpecialProperties.homing)!=0){endWay++;}

				Vector2 prevPos=self.transform.position;
				if(midHit!=null && perent.hasProperties(SpecialProperties.homing)){
				...
				if(endHit==null){
					if((self.transform.position-r.GetPosition(segments)).magnitude>1){
						float t = (endWay+add-1)/(endWay+add);
						Vector2 center = r.GetPosition((int)(t*segments));
						endHit = GetTarget(center,perent.attackRange()/(endWay+add));
					}else{
						endHit = GetTarget(self.transform.position,perent.attackRange());
					}
				}
				if(midHit==null){
					float t = 1/(endWay+add);
					Vector2 center = r.GetPosition((int)(t*segments));
					midHit = GetTarget(center,perent.attackRange()/(endWay+add));
				}
			}
		}
        ...
        public override void Update()
        {
			...
			if(perent.hasProperties(SpecialProperties.random)){
				...
				//yes i fucking know!!!!, but like otherwise this will be a mess and i dont fucking care
				//fuck off and die shitheel
	...
	public class SwordHand : Weapon{
		...
		float swordLength(){
			return 1*perent.attackRange()/3;
		}

		float armLength(){
			return 2*perent.attackRange()/3;
		}
		...
		void ManageSword(){
			if(previousEnd!=Vector3.zero){
				rb.AddForceAtPosition((previousEnd-points[2])/Time.deltaTime,points[2]);
			}
		}
		...
		void SwingSword(){
			...
			if(!AdjustSwordAngle(aimAngle)){
				...
				float newAimDelta=Mathf.Abs(aimAngle-self.transform.eulerAngles.z);
				if(newAimDelta>180){
		...
		public void Cooldown(){
			if(perent.hasProperties(SpecialProperties.returning) && initialAngle!=-1){
				aimDelta=1000;
		...
		public Vector3 Target(){
			if(perent.hasProperties(SpecialProperties.random)){
				...
				float r=armLength();
				if(perent.hasProperties(SpecialProperties.returning)){
					r*=(Mathf.Sin(0.25f*Mathf.PI*(Time.time/perent.attackRate(rapidFireShotCount,chargingPercent))*perent.perent.attackRate)+1)/2;
				}
		...
		public override void Update(){
			...
			if(perent.hasProperties(SpecialProperties.random)){
				...
				rot.z=180*(Time.time/perent.attackRate(rapidFireShotCount,chargingPercent))*perent.perent.attackRate;
				self.transform.eulerAngles=rot;
				MoveCenter();
			}else{
				if(c==null && (s==MeleeState.MidAttack || s==MeleeState.Attacking)){
					Cooldown();
				}
				base.Update();
				switch(s){
					case MeleeState.Waiting:
						if(c!=null && perent.hasProperties(SpecialProperties.predictive)){
