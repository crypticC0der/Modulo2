...
public class LaserAttack : RangedAttack {
    public override void AddAttack(Combatant c) { c.AddAttack<LaserAttack>(); }
    ...
    public override void AtFunc(Vector3 d) {
        ...
        void DealWithHit(Collider2D c) {
            ...
            if (hit != null) {
                DmgOverhead(new DamageData { dmg = damage(), direction = d },
                            hit);
            }
            ...
            tlen += d.magnitude;
            if (tlen > attackRange()) {
                basicRay(new Color(1, 0.5f, 0), new Color(1, 0, 0),
                         v3.ToArray());
                return;
            }
        }
        if ((attackProperties() & SpecialProperties.homing) == 0) {
            RaycastHit2D[] rh =
                Physics2D.RaycastAll(perent.transform.position, d,
                                     attackRange(), perent.layerMask(false));
            for (int i = 0; i < rh.Length; i++) {
                DealWithHit(rh[i].collider);
            }
        } else {
            Collider2D[] cols = Physics2D.OverlapCircleAll(
                perent.transform.position, attackRange(),
                perent.layerMask(false));
            for (int i = 0; i < cols.Length; i++) {
                Vector3 dir =
                    cols[i].transform.position - perent.transform.position;
    ...
    public LaserAttack() : base() {
        ...
        dmg = 1;
        disabledProps |=
            SpecialProperties.predictive & SpecialProperties.returning;
    }
}
}
