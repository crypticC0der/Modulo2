...
public class ArrowDespawn : MonoBehaviour{
	...
	public void FixedUpdate(){
		transform.localScale = Vector2.LerpUnclamped(transform.localScale,Vector2.one*range,Time.deltaTime*7);
...
public class PolarMove:MonoBehaviour{
	...
	public void FixedUpdate(){
		...
		// transform.position=new Vector3(Mathf.Sin(rads),Mathf.Cos(rads))*d.magnitude+perent.position;
...
public class RayDespawnTracking :MonoBehaviour {
	...
	public void FixedUpdate(){
		...
		while(aimNode!=null){
			if(aimNode.Value==null){
				...
				if(d.magnitude>length*1.5f){
					...
					nodes++;
					RaycastHit2D[] raycastHits = Physics2D.RaycastAll(prevPos,d,d.magnitude,perent.perent.layerMask(false));
					...
					foreach(RaycastHit2D hit in raycastHits){
						...
						while(searchNode!=null){
							if(searchNode.Value.gameObject==hit.collider.gameObject){
								newHit=false;
								break;
							}
						...
						if(newHit){
							Damageable newDamage=hit.collider.gameObject.GetComponent<Damageable>();
							if(( perent.attackProperties() & SpecialProperties.homing )!=0){
								LinkedListNode<Damageable> dNode = new LinkedListNode<Damageable>(newDamage);
		...
		while(aimNode!=null){
			...
			i++;
			dataScaled.direction=aimNode.Value.transform.position-data.sender.transform.position;
		...
		if(currentLength>length*1.5f + ((nodes-1)*4f) || !remainingEnemies){
			Destroy(gameObject);
		}
