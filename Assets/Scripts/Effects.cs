...
public static class Effects {
    ...
    public static void Explode(Vector3 center, float strength, Damager sender,
                               int mask = -1, float radius = -1) {
        // generate particles
        ...
        foreach (Collider2D col in c) {
            ...
            if (D) {
                ...
                DamageData d =
                    new DamageData { dmg = 10 * strength,
                                     direction = D.transform.position - center,
                                     properties =
                                         DamageProperties.bypassArmor };
                if (sender == null) {
                    D.TakeDamage(d);
                } else {
        ...
        curve.curve =
            new AnimationCurve(new Keyframe(0, 0.8f, -0.8f, -0.8f),
                               new Keyframe(1, 0.8f / 10, -0.8f, -0.8f));
        sizemod.size = curve;
        ...
        ParticleSystemRenderer pr =
            particleSim.GetComponent<ParticleSystemRenderer>();
