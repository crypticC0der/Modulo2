...
public class ArrowAttack : AreaAttack {
    public override void AddAttack(Combatant c) { c.AddAttack<ArrowAttack>(); }
    ...
    public override Vector3 AtFunc(GameObject o) {
        ...
        DmgOverhead(new DamageData { dmg = damage(), direction = direction },
                    d);
    ...
    public override void SingleAtFunc() {
        GameObject a =
            MeshGens.MinObjGen(Shapes.spikedHexagonOuter, MatColour.blue2);
        a.layer = 7;
        Vector3 p = center;
        p.z += 10;
