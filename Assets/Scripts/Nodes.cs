...
public class Node {
    ...
    public override int GetHashCode() { return (hc).GetHashCode(); }
}
    ...
    public NodePerent(HexCoord hc)
        ...
        if (!hc.IsRectCenter()) {
            throw(new ArgumentException(
                ...
                World.optimizationCubeSize + " attempted to be placed at " +
                hc.ToString()));
        }
    }
        ...
        if (HasState(NodeState.big)) {
            ...
            for (int i = 0; i < World.optimizationCubeSize; i++) {
                for (int j = 0; j < World.optimizationCubeSize; j++) {
                    ...
                    n.ChangeState(NodeState.placeholder, ChangeStateMethod.Off,
                                  pos);
                }
            }
        }
