...
namespace Modulo {
public enum DeckTypes { StackDeck, QueueDeck, ArrayDeck, HashDeck }
...
public class StackDeck : Deck {
    ...
    public StackDeck() { data = new Stack<Item>(); }

    public Item Take(int i) { return data.Pop(); }
        ...
        if (data.Count > 0) {
            ...
            for (int i = 0; i < data.Count; i++) {
                ...
                o.transform.localPosition =
                    new Vector3(offset + (i * minwidth), -7, 0);
            }
...
public class QueueDeck : Deck {
    ...
    public QueueDeck() { data = new Queue<Item>(); }

    public Item Take(int i) { return data.Dequeue(); }
...
public class ArrayDeck : Deck {
    ...
    public ArrayDeck() { data = new Item[count]; }
    ...
    public void Render(GameObject panel) {
        for (int i = 0; i < count; i += 2) {
            GameObject o =
                GameObject.Instantiate(Resources.Load<GameObject>("duoCard"));
            o.transform.SetParent(panel.transform);
            o.transform.localScale = Vector3.one;
            for (int j = 0; j < 2; j++) {
...
public class HashDeck : Deck {
    ...
    public int Hash(Item t) {
        ...
        foreach (char c in t.name) {
            r += ((
                int)(c)-64); // get alphabet value for the charicter a=1 b=2 ...
        }
        return (r % count); // fit it into the hash table
    }

    public HashDeck() { data = new Item[count]; }
    ...
    public void Render(GameObject panel) {
        for (int i = 0; i < count; i += 3) {
            GameObject o =
                GameObject.Instantiate(Resources.Load<GameObject>("tricard"));
            o.transform.SetParent(panel.transform);
            o.transform.localScale = Vector3.one;
            for (int j = 0; j < 3; j++) {
                GameObject c = o.transform.GetChild(j).gameObject;
...
public class TreeDeck : Deck{
        ...
        /// <summary>
        /// the first 16 bits represent the position wheras the next 16 bits
represent the layer
