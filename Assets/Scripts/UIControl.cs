...
public class UIControl : MonoBehaviour {
    ...
    public void Start() {
        inventory =
            new Slider((RectTransform)panel.transform,
                       new Vector3(0, -172.5f, 0), new Vector3(0, -66.5f, 0));
        crafting = new Slider((RectTransform)craftPan.transform,
                              new Vector3(-356, 0, 0),
                              craftPan.transform.position, 2048, 1024);
    ...
    public void Update() {
        ...
        if (!inventory.slidin && !crafting.slidin && !invDown &&
            Input.GetButton("inventory")) {
            crafting.ToStart();
            inventory.Toggle();
        }

        if (!inventory.slidin && !crafting.slidin && !cftDown &&
            Input.GetButton("crafting")) {
            inventory.ToStart();
            crafting.Toggle();
        }
        if (Input.GetButton("place") && !plcDown &&
            PlayerBehavior.holding != null) {
    ...
    class Slider {
        ...
        public Slider(RectTransform trans, Vector3 start, Vector3 end,
                      float initialSpd = 938, float acceleration = 640) {
