...
public class DaggerAttack : MeleeAttack {
    public override void AddAttack(Combatant c) { c.AddAttack<DaggerAttack>(); }

    protected override Weapon basicWeapon() {
        GameObject obj =
            GameObject.Instantiate(Resources.Load<GameObject>("dagger"));
        obj.transform.position =
            perent.transform.position + Vector3.up * 1.525f;
    ...
    public override void Update() {
        ...
        if (increase || allWait) {
            Vector3 d =
                weapons[0].self.transform.position - perent.transform.position;
            ...
            for (int i = 0; i < perent.crossShots; i++) {
                ...
                for (int j = 0; j < perent.funnelShots; j++) {
                    Vector2 direction =
                        new Vector2(Mathf.Sin(r), Mathf.Cos(r)) * d.magnitude +
                        (Vector2)perent.transform.position;
                    weapons[i * perent.funnelShots + j]
                        .self.transform.position = direction;
                    weapons[i * perent.funnelShots + j].start = direction;
                    weapons[i * perent.funnelShots + j]
                        .self.transform.eulerAngles =
                        new Vector3(0, 0, -180 * r / Mathf.PI);
                    weapons[i * perent.funnelShots + j].baseAngle =
                        new Vector3(0, 0, -180 * r / Mathf.PI);
                    // AtFunc(direction);
...
public class ArmAttack : MeleeAttack {
    ...
    public override void AddAttack(Combatant c) { c.AddAttack<ArmAttack>(); }
...
public class SaberAttack : MeleeAttack {
    ...
    public override void AddAttack(Combatant c) { c.AddAttack<SaberAttack>(); }

    protected override Weapon basicWeapon() {
        Beam t = new Beam();
        t.perent = this;
        t.center = perent.transform.position;
        WeaponHitter h = t.self.transform.GetChild(0)
                             .gameObject.AddComponent<WeaponParticle>();
        h.perent = this;
        h.perentLayer = perent.layerMask(false);
