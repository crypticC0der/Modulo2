...
namespace Modulo{
	...
	public class HexCoord{
		...
		public HexCoord RectCenter(){
			return new HexCoord(q&mask,q&mask);
		}

		public override bool Equals(object obj){
			return Equals(obj as HexCoord);
		}

		public bool Equals(HexCoord hc){
			return hc.q==q && hc.r==r;
		}
		...
		public static HexCoord operator *(HexCoord a,int f) => new HexCoord(f*a.q,f*a.r);
	}
	...
	public static class World
	{
		...
		public static void RenderAround(Vector3 v){
			...
			for(int i=-vr;i<vr;i++){
				for(int j=-vr;j<vr;j++){
					NodePerent np = (NodePerent)(HexCoordToNode(hc + new HexCoord(i,j)));
					np.Render();
				}
			}
		}
	...
	public class Node{
		...
		public bool HasState(NodeState ns){
			return (state&ns)!=0;
		}
		...
		public HashSet<Line> Neighbors(){
			...
			if(HasState(NodeState.big)){
				...
				//this checks if a edge is a cube, and then goes through it until its done, you are then left with the offset pointing to the corner
				...
				//starts at the bottom left (1 after the corner), ends up at the bottom right corner (unfocused)
				...
				offset.q-=1;
				//goes from the first non corner at the top right to the left top corner
				RunEdge((int update) => {offset.q-=update;});
				...
				Focus = World.HexCoordToNode(hc+offset);
				rb.over.Add(Focus);
				//gets the next focus and adds it to the corners over, then moves all the way to the last courner without adding it
				...
				for(int i=0;i<3;i++){
					Line a = new Line(World.HexCoordToNode(hc + World.Neighbors[i]));
					Line b = new Line(World.HexCoordToNode(hc - World.Neighbors[i]));
					ret.Add(a);
					ret.Add(b);
				}
				for(int i=0;i<3;i++){
					...
					Line a = new Line(World.HexCoordToNode(hc + World.Neighbors[i+3]));
					a.over.Add(World.HexCoordToNode(hc - over1));
					a.over.Add(World.HexCoordToNode(hc - over2));

					Line b = new Line(World.HexCoordToNode(hc - World.Neighbors[i+3]));
	...
	public class NodePerent: Node{
		...
		public NodePerent(HexCoord hc) : base(hc,NodeState.placeholder | NodeState.big){
			if(!hc.IsRectCenter()){
				throw(new ArgumentException("placeholder nodes must be at a multiple of " + World.optimizationCubeSize));
			}
		}

		public bool Generate(){state&=~NodeState.placeholder;return true;}
		public void Render(){
			if(HasState(NodeState.placeholder)){
				if(!Generate()){
					return;
			...
			if(rectUpdated && !HasState(NodeState.big)){
				for(int i=0;i<World.optimizationCubeSize;i++){
					for(int j=0;j<World.optimizationCubeSize;j++){
						if(World.HexCoordToNode(hc+new HexCoord(i,j)).state!=state){
							return;
						}
					}
				}
