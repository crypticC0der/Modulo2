...
public class TeslaAttack : AreaAttack {
    public override void AddAttack(Combatant c) { c.AddAttack<TeslaAttack>(); }

    public static Vector3[] Zig(Vector3 start, Vector3 end) {
        Vector3[] r = new Vector3[2];
        Vector3 direction = end - start;
        direction =
            Random.Range(-0.3f, 0.3f) * new Vector3(-direction.y, direction.x);
        Vector3 point =
            Vector2.LerpUnclamped(start, end, Random.Range(0.2f, 0.8f));
        r[0] = (direction + point);
        r[1] = (end);
        return r;
    }
    ...
    public override Vector3 AtFunc(GameObject o) {
        ...
        RaycastHit2D[] rh = Physics2D.RaycastAll(center, d, attackRange(),
        ...
        void HitTarget(Damageable hit) {
            ...
            DmgOverhead(
                new DamageData { dmg = damage(), direction = zigs[1] - zigs[0],
        ...
        if (rh.Length > 0) {
            ...
            if ((attackProperties() & SpecialProperties.homing) != 0) {
                Collider2D[] hits = Physics2D.OverlapCircleAll(
                    previous, 2, perent.layerMask(false));
                int c = 0;
            ...
            basicRay(new Color(0.98f, 0.92f, 0.68f),
                     new Color(0.98f, 0.8f, 0.14f), hitPos.ToArray());
        }
        return d;
    }
