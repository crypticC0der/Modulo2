...
public class GasBomb : MonoBehaviour {
    ...
    public void FixedUpdate() {
        ...
        if (scale > 2) {
            ...
            for (int i = 0; i < 3; i++) {
                gasBombs[i + 1] =
                    GameObject.Instantiate(gameObject).GetComponent<GasBomb>();
            }
            for (int i = 0; i < 4; i++) {
                ...
                gasBombs[i].rb.velocity =
                    Vector2.Lerp(velocity.normalized, direction, 0.2f) *
                    velocity.magnitude * gasBombs[i].spd;
    ...
    public void OnTriggerStay2D(Collider2D c) {
        if ((1 << c.gameObject.layer & perentLayer) != 0) {
            ...
            if (d != null) {
                ...
                float multiplier =
                    Time.deltaTime * 2 * Mathf.Min(initialDensity, 0.5f) /
