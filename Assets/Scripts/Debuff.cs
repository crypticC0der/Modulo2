...
namespace Modulo {
public enum DebuffName { Burning = 1, GreenTrail = 2, Stunned = 4, Slowed = 8 }
...
public abstract class Debuff {
    ...
    public virtual void onEnd() { perent.appliedDebuffs ^= name; }
}
...
public class Burning : Debuff {
    ...
    public override bool onUpdate() {
        perent.TakeDamage(
            new DamageData { dmg = dmg * stacks * Time.deltaTime });
...
public abstract class Trail : Debuff {
    ...
    public override bool onUpdate() {
            ...
            if (head > 2) {
                for (int i = 0; i < length; i++) {
                    ...
                    if (p == Vector3.zero) {
                        ...
                        lr.positionCount = i;
                        lr.endWidth =
                            Mathf.Lerp(width[0], width[1], i / (float)length);
                        break;
                    } else {
                        // set the position
                        lr.SetPosition(i, p);
        ...
        if (colTimer > 1 / 30f) {
            DamageData data =
                (new DamageData { dmg = dmg * stacks * colTimer });
            colTimer = 0;
            Globals.DealDmgAlongLine(points, head, mask, (Damageable d) => {
                d.TakeDamage(data);
            }, 69, length);
        }
        t += Time.deltaTime;
