...
public class OnExplode : Proc {
    public override void OnProc(Damageable d) {
        ...
        Effects.Explode(d.transform.position, Mathf.Max(dmg / 10, 0.3f), this,
                        perent.perent.layerMask(false), r);
    }
    ...
    public override Proc newSelf() { return new OnExplode(); }
}
...
public class PullIn : Proc {
    public override void OnProc(Damageable d) {
        ...
        Collider2D[] c = Physics2D.OverlapCircleAll(
            d.transform.position, r, perent.perent.layerMask(false));
        foreach (Collider2D col in c) {
            if (col.gameObject != d.gameObject) {
                ...
                if (D && rb) {
                    ...
                    Vector2 direction =
                        col.transform.position - d.transform.position;
    ...
    public override Proc newSelf() { return new PullIn(); }
}
...
public class LaserBurst : Proc {
        ...
        while (hits < lasers && i < c.Length) {
            if (c[i].gameObject != d.gameObject) {
                ...
                if (D) {
                    ...
                    points[1] = TeslaAttack.Zig(d.transform.position,
                                                c[i].transform.position)[0];
                    points[2] = c[i].transform.position;
                    DmgOverhead(
                        new DamageData {
                            dmg = perent.perent.damage() * dmgMultiplier,
    ...
    public override Proc newSelf() { return new LaserBurst(); }
}

public class PoisonTrail : Proc {
    public override void OnProc(Damageable d) {
        base.OnProc(d);
        d.ApplyDebuff<GreenTrail>(
            perent.perent,
            (int)(perent.perent.damage() * dmgMultiplier / GreenTrail.dmg));
    }
    public PoisonTrail() {
        procCoefficent = 0;
        chance = 0.2f;
    ...
    public override Proc newSelf() { return new PoisonTrail(); }
}
...
public class OnExplodeDelay : Proc {
    ...
    public override Proc newSelf() { return new OnExplodeDelay(); }
}
...
public class PullOut : Proc {
    ...
    public override Proc newSelf() { return new PullOut(); }
}
...
public class PushOut : Proc {
    public override void OnProc(Damageable d) {
        ...
        if (rb) {
            float magnitude = perent.perent.damage() / 1.4f;
            Vector2 direction =
                d.transform.position - perent.perent.transform.position;
            direction = direction.normalized * magnitude;
            rb.AddForce(direction, ForceMode2D.Impulse);
        }
    }
    ...
    public override Proc newSelf() { return new PushOut(); }
}
