...
public class BeamAttack : RangedAttack {
    ...
    public override void AddAttack(Combatant c) { c.AddAttack<BeamAttack>(); }
    ...
    public override void AtFunc(Vector3 d) {
        ...
        if (!rdt) {
            ...
            rdt = basicBeam(new Color(.96f, 0.66f, .72f),
                            new Color(1, .5f, .3f), damageables, len,
                            new DamageData { dmg = damage() * 2, direction = -d,
                                             sender = perent });
        }
    }
